...
export type {FoodSearchItem} from "@/services/food/queries";
...
export function FoodSearchCombobox({
    ...
    return (
        <Popover opened={open} onChange={setOpen} width="target" position="bottom-start" withinPortal>
            <Popover.Target>
                <Button
                    type="button"
                    variant="outline"
          ...
          <span className="truncate text-left">
            ...
            <Popover.Dropdown p={0}>
                ...
                <div className="max-h-72 overflow-y-auto p-1">
                    ...
                    {!isLoading && debouncedQuery && (isError || items.length === 0) && (
                        <p className="px-3 py-6 text-center text-sm text-muted-foreground">
                            {isError ? "Unable to search foods." : emptyMessage}
                        </p>
                    )}

                    {!isLoading && items.length > 0 && (
                        <div>
                            <p className="px-2 py-1 text-xs font-medium text-muted-foreground">Foods</p>
                            {items.map((food) => (
                                <button
                                    key={food.id}
                                    type="button"
