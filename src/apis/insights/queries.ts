...
import { getMealsByRange } from "@/apis/meal/queries";

async function getTopFood(): Promise<{ title: string; foodCount: number }> {
    const session = await getSession();
    const response = await apiFetch<{ title: string; foodCount: number }>(
        "/api/v1/insights/top-food",
        {
            method: "GET",
            headers: {
